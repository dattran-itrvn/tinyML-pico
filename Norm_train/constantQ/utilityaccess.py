...
def canonical_name(name):
    ...
    try:  # use equivalence introduced in scipy 0.16.0
        ...
        return scipy_windows._windows._win_equiv[name.lower()].__name__
...
#         if name.lower() == 'hann':
#             return scipy_windows.hann.__name__
#         else:
#             return scipy_windows.hann.__name__
#     except KeyError:  # no match
#         raise ValueError('no window function in scipy.signal equivalent to %r'
...
#         return 'planck'
#     try:
#         window_func = get_window(name, 10)  # Using a window length of 10 as an example
#         return window_func.__name__
#     except ValueError:
#         raise ValueError('no window function in scipy.signal equivalent to %r' % name)

# from scipy.signal.windows import get_window
# import numpy as np
# # Create a dictionary to map window names to functions
# window_mapping = {
#     'hann': np.hanning,
#     'hamming': np.hamming,
#     'blackman': np.blackman,
#     # Add more window names and functions as needed
# }
#
#
# def canonical_name(name):
...
#         return 'planck'
#
#     window_func = window_mapping.get(name.lower())
#     if window_func:
#         return window_func.__name__
#     else:
#         raise ValueError(f"{name} is not a recognized window name.")
