...
static inline pio_sm_config camera_pio_get_shift_byte_sm_config(PIO pio, uint sm, uint offset, uint base_pin, uint bpp)
{
    ...
    sm_config_set_in_pins(&c, base_pin + PIN_OFFS_D0);
    sm_config_set_in_pins(&c, base_pin + PIN_OFFS_D1);
    sm_config_set_in_pins(&c, base_pin + PIN_OFFS_D2);
    sm_config_set_in_pins(&c, base_pin + PIN_OFFS_D3);
    sm_config_set_in_pins(&c, base_pin + PIN_OFFS_D4);
    sm_config_set_in_pins(&c, base_pin + PIN_OFFS_D5);
    sm_config_set_in_pins(&c, base_pin + PIN_OFFS_D6);
    sm_config_set_in_pins(&c, base_pin + PIN_OFFS_D7);
...
{
    pio_sm_set_consecutive_pindirs(pio, sm, base_pin, 1, false);
    for (uint i = 0; i < 11; i++) {
        pio_gpio_init(pio, i + base_pin);
    }
     pio->input_sync_bypass = (1 << (base_pin + PIN_OFFS_D0));
     pio->input_sync_bypass = (1 << (base_pin + PIN_OFFS_D1));
     pio->input_sync_bypass = (1 << (base_pin + PIN_OFFS_D2));
     pio->input_sync_bypass = (1 << (base_pin + PIN_OFFS_D3));
     pio->input_sync_bypass = (1 << (base_pin + PIN_OFFS_D4));
     pio->input_sync_bypass = (1 << (base_pin + PIN_OFFS_D5));
     pio->input_sync_bypass = (1 << (base_pin + PIN_OFFS_D6));
     pio->input_sync_bypass = (1 << (base_pin + PIN_OFFS_D7));
}
