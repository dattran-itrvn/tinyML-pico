...
def getdata(DATA_DIR_STR, DIR_FOLDER):
    signal_list = []
    label_list = []
    sample5s = 128 * 5
    for index in DATA_DIR_STR:
...
if __name__ == '__main__':
    ...
    VALID_DATA_DIR_STR = [100, 103, 105, 111, 113, 117, 121, 123, 200, 202, 210, 212, 213, 214, 219, 221, 222, 228, 231,
                          232, 233, 234]
    train_data, train_label = getdata(DATA_DIR_STR=TRAIN_DATA_DIR_STR, DIR_FOLDER=DATA_DIR_FOLDER)
    valid_data, valid_label = getdata(DATA_DIR_STR=VALID_DATA_DIR_STR, DIR_FOLDER=DATA_DIR_FOLDER)
    ...
    model = tf.keras.models.Sequential([
        tf.keras.layers.Input(shape=(10, 80, 1)),
        tf.keras.layers.Conv2D(1, kernel_size=(6, 41), strides=1, activation='relu'),
        tf.keras.layers.Conv2D(1, kernel_size=(5, 1), strides=1, activation='relu'),
    ])
    model.summary()
